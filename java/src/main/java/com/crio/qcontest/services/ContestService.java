...
public class ContestService{
    ...
    public Contestant createContestant(Long contestId, String userName) {
        ...
        Contest contest = contestRepository.findById(contestId).orElseThrow(() -> new RuntimeException("Contest with title: "+ contestId +" not found!")); 
        if(contest.getContestStatus()==ContestStatus.ENDED || contest.getContestStatus()==ContestStatus.IN_PROGRESS)
            throw new RuntimeException("Contest " + contest.getContestStatus());
        
        Contestant c=new Contestant(user, contest);
        Contestant savedContestant = contestantRepository.save(c);
    ...
    public List<Contestant> contestHistory(Long contestId) {
        ...
        if(c.getContestStatus()!=ContestStatus.ENDED)
            throw new RuntimeException("Contest with title: " + c.getTitle() + " has not ended");
        List<Contestant> contestantList = contestantRepository.findAllByContestId(contestId);
        return contestantList.stream().sorted((c1,c2)-> c2.getCurrentContestPoints() - c1.getCurrentContestPoints()).collect(Collectors.toList());
    }  
}
